...
void spe_Int(TString f_1 ="file", Bool_t verbose=true){
    ...
    TH1F* h_asympeakInt  = new TH1F("h_asympeakInt","h_asympeakInt", 100, 10, 10);

    TCanvas *c0 = new TCanvas("c0", "asymmetric peak", 1600, 600);
    c0->Divide(2,1);
    c0->cd(1);
    gPad->SetLogy();
    tree1->Draw("asympeakInt>> h_asympeakInt");
    ...
    Int_t nfound = s->Search(h_asympeakInt, 3, "", 0.001);
    if (verbose) printf("Found %d candidate peaks to fit\n",nfound);

    
    TF1 *fline = new TF1("fline","expo",0.01,0.35);
    h_asympeakInt->Fit("fline","");
    ...
    TH1 *hb = s->Background(h_asympeakInt,40,"same");
    hb->SetLineStyle(5);
    if (hb) c0->Update();
    ...
    for (Int_t p=0;p<nfound;p++) {
        ...
        Int_t bin = h_asympeakInt->GetXaxis()->FindBin(xp);
        Double_t yp = h_asympeakInt->GetBinContent(bin);
        //if (yp-TMath::Sqrt(yp) < fline->Eval(xp)) continue;

        //par[3*npeaks+0] = yp; // "height"
        //par[3*npeaks+1] = xp; // "mean"
        //par[3*npeaks+2] = 3.6e-2; // "sigma"

        //this is if I use the background fit  
    ...
    c0->cd(2);
    TH1F *h2 = (TH1F*)h_asympeakInt->Clone("h2");
    TF1 *fit = new TF1("fit",fpeaks,0,0.02,2+3*npeaks);
    //TF1 *fit = new TF1("fit",fpeaks,0,0.35,3*npeaks);
    fit->SetLineColor(4);
